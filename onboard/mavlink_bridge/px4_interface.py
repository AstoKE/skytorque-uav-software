...
class PX4Interface:
    ...
    def connect(self):
        ...
        hb = None
        while True:
            msg = self.master.recv_match(type="HEARTBEAT", blocking=True, timeout=15)
            if msg is None:
            ...
            # QGC / local companion / kendimiz gibi kaynakları ele
            if src_sys == 255:
                continue
    ...
    def get_message(self, msg_types=None, timeout=5, only_target=False):
        while True:
            msg = self.master.recv_match(type=msg_types, blocking=True, timeout=timeout)
    ...
    def read_heartbeat_status(self):
        msg = self.get_message(msg_types=["HEARTBEAT"], timeout=3, only_target=True)
        if msg is None:
            return None
    ...
    def get_relative_altitude(self, timeout=3):
        msg = self.get_message(msg_types=["ALTITUDE"], timeout=timeout, only_target=True)
        if msg is None:
            return None
